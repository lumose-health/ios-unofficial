...
#                clock") are rejected everywhere under Sources/ except
...
EOF
    ...
    # The scope is all of Sources/, matching the constant rules above: a second
    # target directory added later must not silently fall outside the clock rule.
    local scope="$SOURCES" token result sites offenders site
    ...
    # wall-clock read — in ANY forbidden spelling, not only `Date()` — it has
    # started doing time-dependent WORK, which belongs in a type that takes a
    # Clock and can be tested at its boundaries.
    local exempt_reads exempt_total
    if [ "$exemption_present" -eq 1 ]; then
        exempt_total=0
        for token in $CLOCK_FORBIDDEN; do
            exempt_reads=$(count_in_file_clock "$ROOT/$CLOCK_EXEMPT_FILE" "$token" 0)
            exempt_total=$((exempt_total + exempt_reads))
        done
        if [ "$exempt_total" -ne 1 ]; then
            bad "$CLOCK_EXEMPT_FILE must contain exactly one wall-clock read, found $exempt_total — the exemption is an adapter, not a home for logic"
        else
...
EOF
    ...
    total=$((total + 1))
    # The exemption's "exactly one read" rule must count every forbidden spelling,
    # not just `Date()` — otherwise the one exempted file can quietly accumulate
    # clock reads written another way.
    self_test_structural "date-now-read-in-exemption" FAIL "printf 'let extra = Date.now\\n' >> \"\$T/$CLOCK_EXEMPT_FILE\"" || bad_cases=$((bad_cases + 1))
    total=$((total + 1))
    self_test_structural "reference-date-read-in-exemption" FAIL "printf 'let extra = Date.timeIntervalSinceReferenceDate\\n' >> \"\$T/$CLOCK_EXEMPT_FILE\"" || bad_cases=$((bad_cases + 1))
    total=$((total + 1))
    # The clock rule scans all of Sources/, not just Sources/SafetyCore — a
    # future second target must not fall outside it silently.
    self_test_structural "clock-read-outside-safetycore" FAIL \
        "mkdir -p \"\$T/Sources/OtherTarget\" && printf 'import Foundation\\nlet stamp = Date()\\n' > \"\$T/Sources/OtherTarget/Probe.swift\"" \
        || bad_cases=$((bad_cases + 1))
    total=$((total + 1))
